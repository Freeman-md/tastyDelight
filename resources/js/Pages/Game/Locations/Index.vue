<template>
  ...
  <div class="fixed grid grid-cols-2 gap-6 top-36 left-10">
    <Link 
      v-for="(location, index) in locations"
      :key="index"
      :href="route('locations.show', { location })" class="flex flex-col items-center text-lg font-semibold border-b-4 border-l-4 border-dark-green button button-green button-xlg"
    >
      <span class="text-4xl font-bold text-white fa fa-map-marker-alt"></span>
      <span>{{ location.name }}</span>
    </Link>
  </div>
  
  <div class="fixed flex flex-col items-center text-lg font-semibold border-b-4 border-l-4 border-dark-orange button button-orange button-xlg bottom-48 right-24">
